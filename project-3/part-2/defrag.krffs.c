...
int main(int argc, char **argv)
{
    ...
    struct krffs_node *root = file_system.node;
    struct krffs_node *node = krffs_get_next_node(&file_system, file_system.node);

    while (node != root) {
        struct krffs_node *next_node = krffs_get_next_node(&file_system, node);
        if (next_node == root) {
            break;
        }
        if (node->type == KRFFS_Free_Node) {
            if (next_node->type == KRFFS_Free_Node) {
                ...
                uint64_t free_size = node->size;
                uint64_t prev_size = node->previous_node_size;
                memmove(node, next_node, next_node->size);
                node->previous_node_size = prev_size;
                krffs_sync_mapping(node, sizeof(*node));
                
                next_node = krffs_get_next_node(&file_system, node);
                next_node = krffs_initialize_free_node(next_node, free_size, node->size);
                
                struct krffs_node *next_next_node = krffs_get_next_node(&file_system, next_node);
                next_next_node->previous_node_size = next_node->size;
