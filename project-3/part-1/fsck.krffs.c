...
int main(int argc, char **argv)
{
    ...
    if (file_system.node->magic != KRFFS_File_System_Magic) {
        fprintf(
            stderr,
            "The file system was not found at '%s'. The file might be corrupted.\n",
            path
        );
    ...
    if (file_system.node->type != KRFFS_Root_Node) {
        fprintf(
            stderr,
            "The root node was not found at '%s'. The file might be corrupted.\n",
            path
        );
    ...
    // TODO
    struct krffs_node *cn = file_system.node;

    uint64_t size = 0;

    while (1) {
        struct krffs_node *pn = cn;
        cn = krffs_get_next_node(&file_system, cn);
        
        printf(
            "Found a Node with type %s and size %lu\n",
            cn->type == KRFFS_Root_Node ? "Root" : cn->type == KRFFS_Free_Node ? "Free" : "Reserved",
            cn->size
        );

        size += cn->size;

        if (cn == file_system.node) {
            break;
        }
        ...
        if (pn >= cn) {
            ...
            fprintf(
                stderr,
                "Found a nonconsecutive link, the file system is corrupted\n"
            );
            goto cleanup;
        }
        
        if (!krffs_is_node_in_file_system(&file_system, cn)) {
            exit_status = 
                KRFFS_Out_of_Range_Node_Error;
            fprintf(
                stderr,
                "Link is out of the file system range, the file system is corrupted\n"
            );
            goto cleanup;
        }

        if (cn->magic != KRFFS_File_System_Magic) {
            exit_status = 
                KRFFS_Invalid_Magic_Signature_Error;
            fprintf(
                stderr,
                "Found a node with an invalid signature, the file system is corrupted\n"
            );
            goto cleanup;
        }

        if (cn->type != KRFFS_Free_Node && cn->type != KRFFS_Reserved_Node) {
            exit_status = 
                KRFFS_Unknown_Node_Type_Error;
            fprintf(
                stderr,
                "Found a node with an unknown type, the file system is corrupted\n"
            );
            goto cleanup;
        }
    }
    ...
    printf(
        "File System check completed, total size is %lu\n",
        size
    );
